...
#pragma warning restore CS8618 // Non-nullable field must contain a non-null value when exiting constructor. Consider declaring as nullable.
        ...
        {                                                                                             
            var result = await _image_Service.AddSingleImage(image.album_id, image.image_capture_date, image.image_captured_by, image.image_tags, image.geolocation);
        ...
        {
            var result = await _image_Service.UpdateImage(image.image_id, image.album_id, image.image_capture_date, image.image_captured_by, image.image_tags, image.geolocation);
            switch (result.success)
            {
                case true:
                    return Ok(result);
